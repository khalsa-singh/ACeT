"""ACeT mouse-exposure development-only sensitivity analysis.

Run with the model-ready development and raw held-out CSVs supplied in this
addendum. The scientific implementation below is unchanged from the locked
analysis source; only an introductory local command-history block was removed.
"""
import os
import random
import argparse
...
def run_regression(args):
    ...
    synthetic_data = synthesizer.sample(num_rows=63)
    ...
    for head in heads:
        ...
        # Inset box with R2 and nRMSE (upper-left corner)
        ax.text(
            0.05, 0.95,
            f"$R^2$ = {r2:.2f}\nnRMSE = {nrmse:.2f}",
...
def parse_args():
    ...
    p.add_argument('--ff_dim', type=int, default=48)
    ...
    p.add_argument('--min_lr', type=float, default=1e-12)
    return p.parse_args()
